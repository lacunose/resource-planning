...
@section('title')
  <div class="row align-items-center">
    ...
    <div class="col-md-4 text-right">
      <a href="{{ route('tcust.program.saving', [$data['data']['uuid']]) }}" class="btn btn-light">TAMBAH</a>
    </div>
  </div>
@endsection
    ...
    <div class="card-header">
      ...
      <ul class="nav nav-tabs" role="tablist">
        @foreach($data['stats']['status'] as $k => $v)
          <li class="nav-item">
            <a href="{{ route('tcust.program.index', ['status' => $k, 'sort[code]' => 'asc']) }}" class="nav-link
              @if(Str::is(request()->segment(4), $k)) active @endif">{{ $v }}</a>
          </li>
        @endforeach
      </ul>
      <div class="clearfix">&nbsp;</div>
      {!! Form::open(['url' => route('tcust.program.index', ['status' => request()->segment(4), 'sort[code]' => 'asc']), 'class' => 'mb-3', 'method' => 'get']) !!}
